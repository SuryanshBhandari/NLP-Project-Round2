...
#Removing Stopwords from Pride tokens
    ...
    for word in tokens:
        ...
        if lesk(tokens, word).pos() == 'n': # if sense of the word is noun
            ...
            if category not in nouns.keys():
                nouns[category] = 1 # if a category is found for the first time
            else:
                nouns[category] += 1 # incrementing the count of noun category
        elif lesk(tokens, word).pos() == 'v':# if sense of word is verb
            category = lesk(tokens, word).lexname()
            if category not in verbs.keys():
                verbs[category] = 1 # if a noun category is found for the first time
            else:
                verbs[category] += 1 # incrementing the count of noun category
...
#POS Tagging
pos_tagged=nltk.pos_tag(P_tokens_sw)
#print(pos_tagged)
...
findCategories(P_tokens_sw,pos_tagged,nouns,verbs)
...
displacy.render(nlp(Pride_book[2000:4000]), jupyter=True, style='ent')
...
# entity depiction                                             
displacy.render(nlp(Pride_book[3000:3100]), jupyter=True, style='dep')
...
confusionMatrix(results,new,'DATE') # calling confusion matrix function   
confusionMatrix(results,new,'ORG') # calling confusion matrix function   
    
# depicting relationship between entities
pride_sample=Pride_book
pride_sentence = nltk.sent_tokenize(pride_sample)
tokenized_sentences = [nltk.word_tokenize(sentence) for sentence in pride_sentence]
